...
#elif (USE_FREERTOS_TASK || USE_FREERTOS_TASK_MPU)
ee_u8
core_start_parallel(core_results *res)
{
    /* Initialize Queue */
    res->port.queue = xQueueCreate(4, sizeof(uint16_t)); // crc, crclist, crcmatrix, crcstate
    core_results *res_for_task;
    res_for_task = portable_malloc(sizeof(core_results));
...
//    ee_printf("creating task2\r\n");
    ...
    /* Increment the stack buffer pointer */
    ee_u8 result = xTaskCreateRestricted(&benchTaskParameters, &(res->port.task));
    if (!result)
...
core_stop_parallel(core_results *res)
{
    /* Receive results from the task */
//	ee_printf("waiting for crc\r\n");
    xQueueReceive(res->port.queue, &(res->crc), portMAX_DELAY);
//    ee_printf("Received crc\r\n");
    xQueueReceive(res->port.queue, &(res->crclist), portMAX_DELAY);
//    ee_printf("Received crclist\r\n");
    xQueueReceive(res->port.queue, &(res->crcmatrix), portMAX_DELAY);
//    ee_printf("Received crcmatrix\r\n");
    xQueueReceive(res->port.queue, &(res->crcstate), portMAX_DELAY);
//    ee_printf("Received crcstate\r\n");
    /* Delete task */
    vTaskDelete(res->port.task);
//    ee_printf("deleted task\r\n");
    return 1;
}

static void benchTask( void * pvParameters )
{
    /* Get core_results */
//	ee_printf("starting task\r\n");
    core_results *res = pvParameters;
//    ee_printf("starting task\r\n");
    iterate(res);
    /* Send results to queue */
    xQueueSend(res->port.queue, &(res->crc), 0);
    xQueueSend(res->port.queue, &(res->crclist), 0);
    xQueueSend(res->port.queue, &(res->crcmatrix), 0);
    xQueueSend(res->port.queue, &(res->crcstate), 0);

//    /* Free local copy of res */
//    portable_free(res);

    /* This task has finished and is now waiting to be deleted */
    while(1){
    }
}
